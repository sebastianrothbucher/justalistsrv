const rowsDao = (client, wsId, sortMode, sortCol, skip, limit) => new Promise((resolve, reject) => {
    ...
    client.query('select r.id::int, r.title, r.version::int, r.versiondate, rcc.colid::int, rcc.name, rcc.value from (select id, title, version, versiondate from rec where wsid=$1 and archived=false limit $2) r left outer join (select rc.id as id, rc.colid as colid, rc.recid as recid, c.name as name, rc.value as value from reccol rc join col c on rc.colid=c.id) rcc on r.id=rcc.recid order by r.id, rcc.id', [wsId, limit], (err, dbres) => {
        if (!err) { // deserialize res - can assume ordered
            let res = [];
            let currRow = null;
            dbres.rows.forEach(dbrow => {
                if ((!currRow) || currRow.id !== dbrow.id) {
                    currRow = { id: dbrow.id, title: dbrow.title, version: dbrow.version, versiondate: (dbrow.versiondate ? dbrow.versiondate.getTime() : null), cols: [] };
                    res.push(currRow);
                }
