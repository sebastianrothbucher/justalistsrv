const colsDao = (client, wsId) => new Promise((resolve, reject) => {
    client.query('select c.id::int, c.name, cc.value, cc.color from col c left outer join colchoice cc on c.id=cc.colid where c.wsid=$1 order by c.id, cc.id', [wsId], (err, dbres) => {
